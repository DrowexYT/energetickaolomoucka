...
echo "<!-- Debug: add_sale.php updated April 21, 2025 -->";

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $buyer_name = $conn->real_escape_string($_POST['buyer_name']);
    if (!isset($_POST['deal_maker']) || empty($_POST['deal_maker'])) {
        throw new Exception("Deal maker not provided");
    }
    $deal_maker = trim(strtoupper($conn->real_escape_string($_POST['deal_maker']))); // Normalize to "ADAM" or "DD"
    if (!in_array($deal_maker, ['DD', 'ADAM'])) {
        throw new Exception("Invalid deal maker: $deal_maker");
    }
    ...
    try {
        ...
        $stmt->bind_param("siis", $buyer_name, $amount_paid, $is_paid, $deal_maker);
        $stmt->execute();
        $sale_id = $stmt->insert_id;

        // Update customer's total_paid
        $stmt = $conn->prepare("UPDATE customers SET total_paid = total_paid + ? WHERE name = ?");
        $stmt->bind_param("is", $amount_paid, $buyer_name);
        if (!$stmt->execute()) {
            throw new Exception("Failed to update customer total_paid: " . $stmt->error);
        }
        ...
        foreach ($items as $item) {
            ...
            // Insert into sale_items (sale_price is per can)
            ...
            $per_can_sale_price = $item_sale_price / $quantity;
            $per_can_owner_amount = $item_owner_amount / $quantity;
            $per_can_deal_maker_amount = $item_deal_maker_amount / $quantity;
            $stmt->bind_param("iisiiiii", $sale_id, $drink_id, $drink_name, $quantity, $buy_price, $per_can_sale_price, $per_can_owner_amount, $per_can_deal_maker_amount);
            $stmt->execute();
