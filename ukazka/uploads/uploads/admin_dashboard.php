            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                        ...
                        <td>
                            ...
                            <a href='delete_sale.php?id=" . $row['id'] . "' class='btn delete-btn' onclick='return confirm(\"Are you sure?\");'>Delete</a></td>
...
$result = $conn->query("SELECT s.*, (SELECT COUNT(*) FROM sale_items si WHERE si.sale_id = s.id) as item_count FROM sales s ORDER BY s.purchase_date DESC");
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (!$row['is_profit_split']) {
            ...
            // Handle deal_maker
            $deal_maker = trim(strtoupper($row['deal_maker'])); // Normalize to uppercase
            if ($deal_maker == 'DD') {
